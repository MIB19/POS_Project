...
<body>
			...
			while($rec = $result->fetch_assoc()){
				if($rec['mkat2'] == 'PK'){
					...
					$sqlulang	= "
						...
						where tbarang_paket.mkat2 = '$pkt' && mkat2.mprint = 'EPSON E'";
	...
	<script type="text/javascript">
		...
		if($c == 0){
			...
			$a= 'RUNDLL32 PRINTUI.DLL,PrintUIEntry /y /n "EPSON E"';
			//system($a);
		?>
		...
		?>
		$.post('<?php echo $config->base_url();?>print_pastry/<?php echo $id;?>/<?php echo $id1;?>.html',function(data){
			$('#utk_print').html(data);
		});
	</script>
</body>