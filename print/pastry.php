...
<body>
	<?php
		$sql	= "
			...
			WHERE 
				transaksi_detail.stat_simpan = '0' && transaksi_detail.id_trans = '$id' && transaksi_detail.deleted = '0' && mkat2.mprint = 'EPSON D'
	...
	<script type="text/javascript">
		...
		<?php 
			...
			$a = 'RUNDLL32 PRINTUI.DLL,PrintUIEntry /y /n "EPSON D"';
			//system($a);
		?>
		...
		?>
		$('#utk_print').html("");
		$.post('<?php echo $config->base_url();?>simpan_order/<?php echo $id;?>.html',function(data){
			$.post('<?php echo $config->base_url();?>list_transaksis/<?php echo $id1;?>.html',function(data){
				$('#dunia').html(data);
			});
		});
	</script>
