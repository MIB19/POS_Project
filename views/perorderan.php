...
<?php 
	$dt111 = date('Y-m-d', (time()+18000));
	$sqlso = "SELECT shift,status FROM `shift` where date_add LIKE '".$dt111."%' order by id desc limit 1";
	$resultso	= $conn->query($sqlso);
	$recso = $resultso->fetch_assoc();
	if($recso['status'] == null || $recso['status'] == 0){
		if($page_psh == 1){
		}else{
		?>
		<meta http-equiv='refresh' content="0; URL=<?php echo $config->base_url.'pilih_shift.html';?>">
		<?php
		}
	}
?>
<div id='oyicak'>
	<div id="pilihan_card" style="margin-left:1%;margin-right:1%;margin-top:1%;display:block;">
		...
		<div>
			<div style= "width:100%;">
				<div class="card_pilihan_custom" style="height:132px;">
					<a href="<?php echo $config->base_url()."";?>index/107.html">
						<div class="card_dalam_barssss" 
							...
							<?php if($bbb[107] != null){ ?> style="margin: auto;margin: auto;width: 40%;text-align: center;margin-top: 10px;float:left;margin-left: 20%;margin-right: 20%;"
								...
								<img src="<?php echo $config->base_url()."";?>images/owner.png" style="width:100%;display: block;margin-left: 0%;margin-top: -20%;"/>
							</div>
							<div style="margin: auto;width: 10%; text-align: center;float:left;">
					...
					<a href="<?php echo $config->base_url()."";?>deposite_member.html">
						<div class="card_dalam_barssss" style="width:80%;height:38%;float:left;margin-left:5%;margin-top:7%;">
							<div style="margin: auto;width: 50%;text-align: center;margin-top:6px;">
								<img src="<?php echo $config->base_url()."";?>images/deposit.png" style="height:80%;width:80%;display: block;margin-left: auto;margin-right: auto;"/>
							</div>
						</div>
					</a>
					...
					<?php 
						$dt111 = date('Y-m-d', (time()+18000));
						$sqlso = "SELECT * FROM `shift` where date_add LIKE '".$dt111."%' order by id desc limit 1";
						$resultso	= $conn->query($sqlso);
						$recso = $resultso->fetch_assoc();
					?>
					...
					<div style="float:left">
						<a 
							<?php if($recso['shift'] == '1' && $recso['status'] == '1'){ ?> href="<?php echo $config->base_url()."";?>index/shift01.html" 
							<?php }else{ ?>href="<?php echo $config->base_url()."";?>index/shift1.html" <?php } ?>
						>
							<div class="card_dalam_barssss" 
								<?php if($recso['shift'] == '1' && $recso['status'] == '1'){ ?> 
									style="width:45px;height:50px;float:left;margin-left:5%;margin-top:20%;margin-right:4px;background:#D84949;color:white;"
								<?php }else{?>
									style="width:45px;height:50px;float:left;margin-left:5%;margin-top:20%;margin-right:4px;"
								<?php }?>
								>
								<div style="margin: auto;width: 100%;text-align: center;margin-top: 10px;font-size:8px;font-weight:bold;">
									SHIFT 1
								</div>
							</div>
						</a>
					...
					<div style="float:left">
						<a 
							<?php if($recso['shift'] == '2' && $recso['status'] == '1'){ ?> href="<?php echo $config->base_url()."";?>index/shift02.html" 
							<?php }else{ ?>href="<?php echo $config->base_url()."";?>index/shift2.html" <?php } ?>
						>
							<div class="card_dalam_barssss" 
								<?php if($recso['shift'] == '2' && $recso['status'] == '1'){ ?> 
									style="width:45px;height:50px;float:left;margin-left:5%;margin-top:20%;margin-left:4px;background:#D84949;color:white;"
								<?php }else{?>
									style="width:45px;height:50px;float:left;margin-left:5%;margin-top:20%;margin-left:4px;"
								<?php }?>
								>
								<div style="margin: auto;width: 100%;text-align: center;margin-top: 10px;font-size:8px;font-weight:bold;">
									SHIFT 2
