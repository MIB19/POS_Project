...
<?php 
	...
	foreach($show_items as $rowa){
		...
		if($cccccc == '0'){
		?>	<script>window.location = '<?php echo $config->base_url();?>index/<?php echo $pilihan_meja; ?>.html';</script><?php
		}
...
<div id="pilihan_utama_<?php echo $id_trans;?>" class="card_table" style="margin-left:1%;margin-right:1%;margin-top:1%;display:block;" >
	...
	<div style="width:100%; text-align: center;padding: 5px;font-size: 13px;margin-top:30px;" >
		...
		<?php }else{ ?>
			...
			<div class="card_pilihan_meja1"style="width:70px;float:right;height:auto;background:#987860;color:white;" id ="bn_mn2<?php echo $id_trans;?>" onClick="btnSubmit<?php echo $id_trans;?>('<?php echo $id_trans;?>')">
				<div style="margin: auto;width: 100%;padding-right: 4px;padding-left: 4px; text-align: center;cursor:pointer;">
					
					<b>Split</b>
				</div>
			</div>
			<div class="card_pilihan_meja1"style="width:70px;float:right;height:auto;background:#987860;color:white;" id ="bn_mn3<?php echo $id_trans;?>" onClick="btnPindah<?php echo $id_trans;?>('<?php echo $id_trans;?>')">
				<div style="margin: auto;width: 100%;padding-right: 4px;padding-left: 4px; text-align: center;cursor:pointer;">
					
					<b>Pindah</b>
				</div>
			</div>
			<div class="card_pilihan_meja1" style="width:70px;float:right;height:auto;background:#987860;color:white;" id ="bn_mn4<?php echo $id_trans;?>" onClick="print_data<?php echo $id_trans;?>('<?php echo $id_trans; ?>')">
				<div style="margin: auto;width: 100%;padding-right: 4px;padding-left: 4px; text-align: center;cursor:pointer;">
					<b>Preview</b>
				</div>
			</div>
		...
		<div style="clear: both;"/>
		<div id="kartu_member_<?php echo $id_trans;?>" style="font-size:14px;float:left;text-align:left;width:100%;display:none;" <?php if($hid == ''){ ?> hidden <?php } ?>>
			...
			<div class="card_pilihan_meja1" onClick="member_cari<?php echo $id_trans;?>()" style="width:30px;height:auto;background:#000000;border-color:#000000;color:white;font-size:12px;border-radius: 4px;margin-top:2px;cursor:pointer;">
			...
			<input type="text" name="saldo_member" id="saldo_member_<?php echo $id_trans;?>" placeholder="saldo_member" required class="form_inputss" style="float:right;text-align:right;" value='<?php echo number_format($deposit); ?>' disabled>
		</div>
		<div style="clear: both;"/>
		...
		<table class="display" style="width:100%; padding-right;10px;" border=1>
			...
			<tbody>
				...
				<tr class="row-select">
					...
					<td class="id" id="qqqq<?php echo $row['id_trans'];?>" hidden><?php echo $row['id'];?></td>
					...
					<td>
						<input type="text" onClick="action_setiap_perintah('kets<?php echo $row['id'];?>')" id="kets<?php echo $row['id'];?>" value="<?php echo $row['keterangan'];?>" style="width:100px;text-align:center;" onkeypress="raaaa('<?php echo $row['id'];?>')" />
					</td>
					...
					<td>
						...
						<?php }else{ echo number_format((($row['hrg']*$row['total'])+$ttlads)*$discount/100); } ?>
					</td>
				...
				<?php if($pilihan_meja != '103' && $pilihan_meja != '104'){ ?>
					<tr>
						...
						<td style="border-bottom-style: dotted;border-left-style: dotted;border-right-style: dotted;"><?php echo number_format($service1);?></td>
						<td colspan=2; style="border-bottom-style: hidden;border-right-style: hidden;border-bottom-style: dotted;border-left-style: dotted;border-right-style: dotted;" id="clspn1_<?php echo $id_trans;?>">
				...
				<?php if($pilihan_meja != '103' && $pilihan_meja != '104'){
					...
					<tr>
						<td colspan=10; style="border-bottom-style: hidden;border-left-style: hidden;" id="clspn_<?php echo $id_trans;?>"></td>
						<td colspan=2; align=left style="border-bottom-style: dotted;border-left-style: dotted;"><div style="color:red;">Voucher</div></td>
						<td style="border-bottom-style: dotted;border-left-style: dotted;border-right-style: dotted;">
		...
		<?php if($ses[2] == 0 || $ses[2] == 1){ ?>
		<div id="pbyrn_<?php echo $id_trans;?>" <?php if($pilihan_meja == '103' || $pilihan_meja == '104'){ ?> style="margin-top:-80px;margin-bottom:100px;width:100%;"<?php }else{ ?> style="margin-top:-100px;margin-bottom:100px;width:100%;"<?php } ?>   <?php if($simpan == 0){ ?> hidden <?php } ?>>
...
<script>
	...
	?>	
			...
			$("#ipt_txt_<?php echo $id_trans;?>").keyup(function(event) {
				if (event.keyCode === 13) {
					var stg_txt_<?php echo $id_trans;?> = document.getElementById("ipt_txt_<?php echo $id_trans;?>").value;
					$.post('<?php echo $config->base_url();?>simpan_nama/'+<?php echo $id_trans;?>+'/'+stg_txt_<?php echo $id_trans;?>+'.html',function(data){
						$.post('<?php echo $config->base_url();?>list_transaksis/'+<?php echo $pilihan_meja;?>+'.html',function(data){
							$('#dunia').html(data);
						});
					});
			...
			<?php if($simpan == '1'){ ?>
				function bayar_sekarang<?php echo $id_trans;?>(){
					...
					if(kembalian<?php echo $id_trans;?> < 0){
						...
						open('<?php echo $config->base_url();?>print_bill/'+<?php echo $id_trans;?>+'/'+kembalian<?php echo $id_trans;?>+'/'+a11+'/'+a12+'.html');
			...
			function print_data<?php echo $id_trans;?>(param){
				if(confirm('Apakah Anda Yakin ?')){
					//$.post('<?php echo $config->base_url();?>print_bill_preview/'+param+'.html',function(data){
					//	$('#utk_print').html(data);
					//});
					 $('#dunia').html('Loading...');
